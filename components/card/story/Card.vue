<template>
  <div class="position-relative card-story-container">
    <div class="card-story card border-0">
      <img
        class="story img-fluid"
        src="data:image/png;base64,"
        alt="story"
      />
      <img
        class="rounded-circle profile border border-1"
        src="data:image/png;base64,"
        alt=""
      />
    </div>
    <div class="username text-truncate">@{{ story.username }}</div>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'
import CardStory from '~/interfaces/card-story'

export default Vue.extend({
  props: {
    story: {
      type: Object as () => CardStory,
      default: () => {},
      required: true,
    },
  },
})
</script>

<style scoped lang="scss">
.card-story-container {
  max-width: 60px;
  margin-right: 8px;

  .card-story {
    max-width: 59px;
    height: 76px;
    box-shadow: 0px 2px 2px rgba(0, 0, 0, 0.25);
    padding: 1px;

    .story {
      height: 100%;
      width: 100%;
    }
    .profile {
      position: absolute;
      top: 8px;
      left: 8px;
      width: 24px;
      height: 24px;
    }
  }
  .username {
    font-size: 12px;
    padding: 3px;
  }
}
</style>
